...
4. a host-side refresh workflow that keeps the download tree current
...
The host install also enables a recurring systemd timer and exposes a small status JSON under the download tree for operational checks.
...
- validation and release helper scripts for repeatable operational releases
