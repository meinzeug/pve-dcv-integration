...
mkdir -p "$DIST_DIR"
rm -f "$DIST_DIR/$ZIP_NAME" "$DIST_DIR/$TARBALL_NAME" "$DIST_DIR/$USB_INSTALLER_NAME" "$DIST_DIR/$USB_INSTALLER_LATEST_NAME" "$DIST_DIR/$CHECKSUM_FILE"
...
(
  cd "$DIST_DIR"
  sha256sum "$ZIP_NAME" "$TARBALL_NAME" "$USB_INSTALLER_NAME" "$USB_INSTALLER_LATEST_NAME" > "$CHECKSUM_FILE"
)
